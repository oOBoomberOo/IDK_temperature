...
execute as @s[nbt={Dimension:0b},scores={tmp.altitude=..15}] if block ~ ~1 ~ #temperature:temp_calculation/sunlight positioned ~ ~1 ~ run function temperature:temp_calculation/is_exposed_to_sun
execute as @s[scores={tmp.altitude=..15}] unless block ~ ~1 ~ #temperature:temp_calculation/sunlight run tag @s add tmp.altitude.no_direct_sunlight
execute as @s[nbt=!{Dimension:0b}] run tag @s add tmp.altitude.no_direct_sunlight
